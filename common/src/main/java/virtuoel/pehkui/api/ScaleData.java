...
public class ScaleData
{
	...
	public float getScale(float delta)
	{
		...
		final Level level = e == null ? null : e.level();
		final boolean canCache = delta == 1.0F && e != null && level != null && !level.isClientSide() && (e.getType() != EntityTypes.PLAYER || !getScaleType().getAffectsDimensions()) && !((PehkuiEntityExtensions) e).pehkui_isFirstUpdate();
	...
	public void markForSync(boolean sync)
	{
		...
		if (e != null && level != null && !level.isClientSide())
		{
	...
	public void readNbt(CompoundTag tag)
	{
		...
		this.persistent = tag.getBoolean("persistent").orElse(null);

		this.easing = tag.getString("easing")
			.map(Identifier::tryParse)
			.map(id -> ScaleRegistries.getEntry(ScaleRegistries.SCALE_EASINGS, id))
			.orElse(null);
		...
		final Optional<ListTag> storedModifiers = tag.getList("baseValueModifiers");

		if (storedModifiers.isPresent())
		{
			final ListTag modifiers = storedModifiers.get();

			Identifier id;
			ScaleModifier modifier;
			for (int i = 0; i < modifiers.size(); i++)
			{
				id = modifiers.getString(i)
					.or(() -> Optional.empty())
					.map(Identifier::tryParse)
					.orElse(null);
				...
				if (id == null)
				{
					id = modifiers.getCompound(i)
						.flatMap(compound -> compound.getString("id"))
						.map(Identifier::tryParse)
						.orElse(null);
				}

				modifier = id == null ? null : ScaleRegistries.getEntry(ScaleRegistries.SCALE_MODIFIERS, id);
	...
	public CompoundTag writeNbt(CompoundTag tag)
	{
		...
		if (easing != null)
		{
			tag.put("easing", StringTag.valueOf(ScaleRegistries.getId(ScaleRegistries.SCALE_EASINGS, easing).toString()));
		}
		...
		if (!this.differingModifierCache.isEmpty())
		{
			...
			for (final ScaleModifier modifier : this.differingModifierCache)
			{
				modifiers.add(StringTag.valueOf(ScaleRegistries.getId(ScaleRegistries.SCALE_MODIFIERS, modifier).toString()));
			}
