...
public class ScaleOperationArgumentType implements ArgumentType<ScaleOperationArgumentType.Operation>
{
	...
	{
		final Identifier id = string.contains(":") ? Identifier.tryParse(string) : Pehkui.id(string);

		final DoubleBinaryOperator entry = id == null ? null : ScaleRegistries.getEntry(ScaleRegistries.SCALE_OPERATIONS, id);
