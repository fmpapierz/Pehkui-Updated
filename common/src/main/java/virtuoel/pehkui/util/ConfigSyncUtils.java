...
import virtuoel.pehkui.api.config.JsonConfigHandler;
...
public class ConfigSyncUtils
{
	...
	public static void syncConfigs(final Collection<ServerPlayer> players)
	{
		...
		{
			syncConfigs(player, syncableEntries());
		}
	}

	public static void syncConfigs(final ServerPlayer player)
	{
		syncConfigs(player, syncableEntries());
	}

	public static void syncConfigs(final ServerPlayer player, final String... configEntryKeys)
	{
		...
		syncConfigs(player, entries);
	}

	public static void syncConfigs(final ServerPlayer player, final Collection<SyncableConfigEntry<?>> configEntries)
	{
		...
		PehkuiPlatform.get().sendToPlayer(player, new ConfigSyncPayload(configEntries));
	}

	private static Collection<SyncableConfigEntry<?>> syncableEntries()
	{
		final List<SyncableConfigEntry<?>> entries = new ArrayList<>();

		for (final SyncableConfigEntry<?> entry : SYNCED_CONFIGS.values())
		{
			if (entry != null)
			{
				entries.add(entry);
	...
	public static Runnable readConfigs(final FriendlyByteBuf buffer)
	{
		...
		for (int i = 0; i < qty; i++)
		{
			...
			{
				Pehkui.LOGGER.warn("Codec not found. Could not parse config \"{}\" from server.", name);
				break;
			}
